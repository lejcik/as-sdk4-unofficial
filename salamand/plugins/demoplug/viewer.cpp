...
void
CRendererWindow::OnContextMenu(const POINT *p)
{
...
BOOL
CViewerWindow::IsMenuBarMessage(CONST MSG *lpMsg)
{
  if (MenuBar == NULL)
    return FALSE;
...
BOOL
CViewerWindow::InitializeGraphics()
{
...
BOOL
CViewerWindow::InsertToolBarBand()
{
  REBARBANDINFO  rbbi;
  ZeroMemory(&rbbi, sizeof(rbbi));
...
void
FillMenuFilter(CGUIMenuPopupAbstract *popup, int cmdFirst, int filterCount)
{
...
CViewerWindow::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg)
  {
    ...
    {
      if (MenuBar != NULL) MenuBar->SetFont();
      if (ToolBar != NULL) ToolBar->SetFont();
      return 0;
    }
    ...
    case WM_KEYDOWN:
    {
      if (wParam == VK_ESCAPE) PostMessage(HWindow, WM_CLOSE, 0, 0);
      ...
      if (wParam == VK_SPACE)
      {
        if (!ctrlPressed && !altPressed)
        {
          ...
          if (ok) OpenFile(fileName); // mame nove jmeno
          else
          {
    ...
    case WM_COMMAND:
    {
      ...
      switch (LOWORD(wParam))
      {
        case CM_VIEWER_OPEN:
        {
          ...
          if (SalamanderGeneral->SafeGetOpenFileName(&ofn)) OpenFile(file);
          break;
        }
...
void
CViewerWindow::OpenFile(const char *name, BOOL setLock)
{
